...
alert(‘jsPDF non charge. Verifiez votre connexion.’);
return;
}
...
doc.text(‘Bilan de Rentabilite Detaille’, 15, 26);
...
drawSection(‘Revenus’, [[‘Prix de vente estime’, fmt(res.prixVente)]]);
drawSection(‘Couts Directs & Frais’, [
[‘Matieres premieres / Marchandises’, fmt(res.matieres)],
[‘Transport / Essence’, fmt(res.transport)],
[‘Petit outillage’, fmt(res.outillage)],
[‘Autres frais’, fmt(res.autresFrais)],
]);
drawSection(‘Main oeuvre’, [
[fields.heures + ’h x ’ + fmt(parseFloat(fields.tauxHoraire) || 0) + ‘/h’, fmt(res.coutMain)],
]);
drawSection(‘Cotisations Sociales’, [
...
doc.setFontSize(8);
doc.text(‘ProfitMaster - Outil de rentabilite pour artisans, freelances & commercants’, 105, 290, { align: ‘center’ });
...
<div>
<div style={{ fontSize: 11, color: ‘#8A95AA’, letterSpacing: ‘0.08em’, textTransform: ‘uppercase’ }}>Sante du projet</div>
...
<button onClick={onClose} style={{ position: ‘absolute’, top: 16, right: 16, background: ‘none’, border: ‘none’, color: ‘#4A5568’, fontSize: 22, cursor: ‘pointer’, lineHeight: 1 }}>X</button>
...
Obtenez votre <strong style={{ color: ‘#4FFFA0’ }}>Bilan de Rentabilite complet en PDF</strong> — pret a partager avec vos clients.
</p>
<div style={{ background: ‘#0A0B0F’, borderRadius: 12, padding: ‘16px 20px’, marginBottom: 24, textAlign: ‘center’ }}>
<div style={{ fontSize: 11, color: ‘#4A5568’, letterSpacing: ‘0.08em’, textTransform: ‘uppercase’ }}>Acces unique</div>
...
style.textContent = `@import url('https://fonts.googleapis.com/css2?family=Syne:wght@700;800;900&family=Inter:wght@400;500;600&display=swap'); * { margin: 0; padding: 0; box-sizing: border-box; } body { background: #0A0B0F; color: #E8EDF5; font-family: Inter, sans-serif; } @keyframes fadeSlide { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } } @keyframes pulse { 0%,100% { box-shadow: 0 0 0 0 rgba(79,255,160,0.3); } 50% { box-shadow: 0 0 20px 6px rgba(79,255,160,0.12); } } input[type=number]::-webkit-inner-spin-button { -webkit-appearance: none; } select { -webkit-appearance: none; appearance: none; }`;
...
```
  <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px' }}>
    <Section title='REVENUS' icon='💰'>
      <InputField label='Prix de vente estime' value={fields.prixVente} onChange={setField('prixVente')} hint='Le montant que vous facturez au client' />
    </Section>

    <Section title='COUTS DIRECTS' icon='📦'>
      <InputField label='Matieres premieres / Marchandises' value={fields.matieres} onChange={setField('matieres')} />
    ...
    <Section title='FISCALITE & COTISATIONS' icon='🏛️'>
      <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#8A95AA', marginBottom: 6, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
        Regime fiscal
      </label>
      ...
      {fields.tauxOption === 'custom' && (
        <InputField label='Taux personnalise (%)' prefix='%' value={fields.tauxPersonnalise} onChange={setField('tauxPersonnalise')} placeholder='Ex: 30' />
      )}
      <div style={{ fontSize: 12, color: '#4A5568' }}>
        Cotisations calculees : <span style={{ color: '#8A95AA' }}>{fmt(res.cotisations)} ({fields.tauxCotisations}% du CA)</span>
      </div>
    </Section>
    ...
    {res.prixVente > 0 && (
      <div style={{ marginTop: 8 }}>
        ...
        <p style={{ textAlign: 'center', fontSize: 11, color: '#2E3545', marginTop: 8 }}>
          Calcul gratuit · Export PDF securise
        </p>
      </div>
    )}
